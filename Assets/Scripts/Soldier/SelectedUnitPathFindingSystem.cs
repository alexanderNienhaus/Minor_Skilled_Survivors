...
public partial class SelectedUnitPathFindingSystem : SystemBase
{
    ...
    protected override void OnUpdate()
    {
        if (Input.GetMouseButtonDown(1) && PathfindingGridSetup.Instance != null)
        {
            ...
            FindSoldierPathJob findGroupPathJob = new FindSoldierPathJob
            {
    ...
    private void SetEndPositions(NativeArray<float3> endPositions)
    {
        for (int i = 0; i < endPositions.Length; i++)
        {
                ...
                in SystemAPI.Query<RefRW<FormationPosition>, RefRO<LocalTransform>>().WithAll<SelectedUnitTag>())
            {
                ...
                float currentDistance = math.length(endPositions[i] - localTransform.ValueRO.Position);
        ...
        foreach (RefRW<FormationPosition> formationPosition in SystemAPI.Query<RefRW<FormationPosition>>().WithAll<SelectedUnitTag>())
        {
